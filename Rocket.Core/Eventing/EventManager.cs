...
namespace Rocket.Core.Eventing
{
    public class EventManager : IEventManager
    {
        ...
        {
            var handler = GetEventHandler(callback.GetType(), null);
            eventListeners.Add(new EventAction(@object, @callback.Invoke, handler, eventName));
        }

        public void Subscribe<TEvent>(ILifecycleObject @object, EventCallback<TEvent> callback, string emitterName = null) where TEvent : IEvent
        {
            var handler = GetEventHandler(callback.GetType(), emitterName);
            eventListeners.Add(new EventAction(@object, (sender, @event) =>
            {
                @callback.Invoke(sender, (TEvent)@event);
            }, handler, typeof(TEvent)));
        }

        public void Subscribe(ILifecycleObject @object, Type eventType, EventCallback callback, string emitterName = null)
        {
            var handler = GetEventHandler(callback.GetType(), emitterName);
        ...
        public void Subscribe<TEvent, TEmitter>(ILifecycleObject @object, EventCallback<TEvent> callback) where TEvent : IEvent where TEmitter : IEventEmitter
        {
            var handler = GetEventHandler(callback.GetType(), GetEmitterName(typeof(TEmitter)));
            eventListeners.Add(new EventAction(@object, (sender, @event) =>
            {
                callback.Invoke(sender, (TEvent)@event);
            }, handler, typeof(TEvent)));
        ...
        {
            var handler = GetEventHandler(callback.GetType(), GetEmitterName(eventEmitterType));
            eventListeners.Add(new EventAction(@object, callback.Invoke, handler, eventType));
        }
        ...
        {
            eventListeners.RemoveAll(c => c.Owner == @object
                                           && c.Handler.EmitterName.Equals(emitterName, StringComparison.OrdinalIgnoreCase)
                                           && CheckEvent(c, eventName));
        }

        public void Unsubscribe<TEvent, TEmitter>(ILifecycleObject @object) where TEvent : IEvent where TEmitter : IEventEmitter
        {
            eventListeners.RemoveAll(c => c.Owner == @object
        ...
        public void AddEventListener(ILifecycleObject @object, IEventListener eventListener)
        {
            ...
            foreach (var @interface in type.GetInterfaces().Where(c => c == typeof(IEventListener<>)))
            {
        ...
        public static string GetEmitterName(Type type)
        {
            return type.Name;
        }
        ...
        {
            var handler = (EventHandler)target.GetCustomAttributes(typeof(EventHandler), false).FirstOrDefault() ??
                          new EventHandler();
        ...
        private bool CheckEvent(EventAction eventAction, string eventName)
        {
            return (eventAction.TargetEventType != null
