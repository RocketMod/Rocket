...
namespace Rocket.API.Handlers
{
    public static class HandlerPriorityComparer
    {
        ...
        {
            var handlerAttribute = (HandlerPriorityAttribute)
                a.GetType().GetCustomAttributes(typeof(HandlerPriorityAttribute), true).FirstOrDefault()
                ?? new HandlerPriorityAttribute {Priority = HandlerPriority.Normal};
